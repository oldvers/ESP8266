    ...
    while (0 != uart_intr_status)
    {
        ...
        if ( 0 != (uart_intr_status & UART_TXFIFO_EMPTY_INT_ST_M) )
        {
            ...
            /*esp_err_t uart_clear_intr_status(uart_port_t uart_num, uint32_t mask)
            {
                UART_CHECK((uart_num < UART_NUM_MAX), "uart_num error", ESP_ERR_INVALID_ARG);
                UART_ENTER_CRITICAL();*/
                //EAST_UART->int_clr.val |= UART_TXFIFO_EMPTY_INT_CLR_M;
                /*UART_EXIT_CRITICAL();
                return ESP_OK;
            }*/
            
 //           printf("IRQ: UART Tx Empty\n");
            /*esp_err_t uart_disable_intr_mask(uart_port_t uart_num, uint32_t disable_mask)
            {
                UART_CHECK((uart_num < UART_NUM_MAX), "uart_num error", ESP_ERR_INVALID_ARG);
                UART_ENTER_CRITICAL();*/
                //EAST_UART->int_ena.val &= ~(UART_TXFIFO_EMPTY_INT_ENA_M));
                /*UART_EXIT_CRITICAL();
                return ESP_OK;
            }*/
...
//CALL TX COMPLETE
                  ...
                  (0 != (uart_intr_status & UART_RXFIFO_FULL_INT_ST_M)) )
        {
            ...
            //if (0 < rx_fifo_len)
            //{
            //    //printf("IRQ: UART Rx Full/Tout. Len = %d\n", rx_fifo_len);
            //    rxxx += rx_fifo_len;
            //}
            ...
            {
//CALL RX BYTE                    p_uart->rx_data_buf[buf_idx++] = uart_reg->fifo.rw_byte;
            ...
            //else
            //{
//                uart_disable_intr_mask(UART_NUM_0, UART_RXFIFO_FULL_INT_ENA_M | UART_RXFIFO_TOUT_INT_ENA_M);
//                uart_clear_intr_status(UART_NUM_0, UART_RXFIFO_FULL_INT_CLR_M | UART_RXFIFO_TOUT_INT_CLR_M);
            //}
        }
        else if (uart_intr_status & UART_RXFIFO_OVF_INT_ST_M)
        {
            // When fifo overflows, we reset the fifo.
            //uart_reset_rx_fifo(UART_NUM_0);
        ...
        {
            EAST_UART->int_clr.val = uart_intr_status; // simply clear all other intr status
            //uart_event.type = UART_EVENT_MAX;
            //notify = UART_SELECT_ERROR_NOTIF;
        }
...
//        return ESP_FAIL;
//    }
    printf("UART: Register ISR\n");
    r = uart_isr_register(UART_NUM_0, EAST_UART_IrqHandler, NULL);
//esp_err_t uart_isr_register(uart_port_t uart_num, void (*fn)(void *), void *arg)
//{
//    UART_CHECK((uart_num < UART_NUM_MAX), "uart_num error", ESP_ERR_INVALID_ARG);
...
//
//    UART_ENTER_CRITICAL();
//    UART[uart_num]->int_ena.val &= ~(UART_INTR_MASK);
//    UART_EXIT_CRITICAL();
//    return ESP_OK;
//}
...
//{
//    UART_CHECK((uart_num < UART_NUM_MAX), "uart_num error", ESP_ERR_INVALID_ARG);
    printf("UART: Clear interrupt status\n");
